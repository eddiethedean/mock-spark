...
Mock-Spark ships with multiple storage backends. Polars is the default engine in
...
- `polars` (default) – fast, thread-safe, zero-install beyond the Mock-Spark
  dependency.
...
- `file` – persists tables under `mock_spark_storage/` as JSON/CSV files. Handy
  for sharing fixtures across sessions.
- `duckdb` (optional) – legacy SQL-backed engine. Requires the
  `mock_spark.backend.duckdb` modules to be installed (available in the 2.x
  releases) alongside `duckdb`/`duckdb-engine` Python packages.

Call `mock_spark.backend.factory.BackendFactory.list_available_backends()` to see
which backends are currently importable in your environment.

## Selecting a Backend

Mock-Spark resolves the backend in the following order:

1. **Explicit constructor argument** – `SparkSession(backend_type="memory")`
2. **Environment variable** – set `MOCK_SPARK_BACKEND=memory` before starting
   the process. This is convenient for CI toggles.
3. **Builder configuration** – `SparkSession.builder.config("spark.mock.backend", "file")`
...
os.environ["MOCK_SPARK_BACKEND"] = "memory"
spark = SparkSession()
...
- **DuckDB import errors** – install Mock-Spark 2.x (which includes the DuckDB
...
- **Permission issues with `file` backend** – adjust the base path by passing
  `SparkSession.builder.config("spark.mock.backend.basePath", "/tmp/mock")` and
