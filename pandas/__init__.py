"""
Lightweight stub implementation of pandas for Mock Spark testing.

This module provides the minimal surface area required by the Mock Spark test
suite without depending on the real pandas (and by extension NumPy) wheels,
which are unavailable in the current environment.
"""
    ...
    def __init__(self, data: object = None, columns: Sequence[str] | None = None):
        ...
        if isinstance(data, DataFrame):
            ...
            if data and isinstance(data[0], dict):
                ...
                rows = []
                for entry in data:
                    rows.append({col: entry[idx] for idx, col in enumerate(columns)})
            ...
            raise TypeError(f"Unsupported data type for stub DataFrame: {type(data)!r}")
