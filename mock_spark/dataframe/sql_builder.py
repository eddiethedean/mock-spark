...
class SQLQueryBuilder:
    ...
    def add_with_column(self, col_name: str, col: Union[MockColumn, MockColumnOperation, MockLiteral]) -> None:
    ...
    def _window_spec_to_sql(self, window_spec: Any) -> str:
        ...
        if hasattr(window_spec, '_rows_between') and window_spec._rows_between:
            ...
            parts.append(f"ROWS BETWEEN {self._frame_bound_to_sql(start)} AND {self._frame_bound_to_sql(end)}")
        elif hasattr(window_spec, '_range_between') and window_spec._range_between:
            start, end = window_spec._range_between
            parts.append(f"RANGE BETWEEN {self._frame_bound_to_sql(start)} AND {self._frame_bound_to_sql(end)}")
