...
from mock_spark.errors import AnalysisException, QueryExecutionException, PySparkRuntimeError
...
class ErrorSimulator:
    ...
    def add_error_rule(self, condition: str, exception_class: type, message: str = None):
    ...
    def should_raise_error(self, operation: str, **kwargs) -> Optional[Exception]:
        ...
        for condition, rule in self.error_rules.items():
            if self._evaluate_condition(condition, operation, **kwargs):
                rule['count'] += 1
                self.error_count += 1
                return rule['exception'](rule['message'])
        return None
...
class PerformanceSimulator:
    ...
    def add_slowdown(self, operation: str, delay_seconds: float):
        ...
        self.performance_rules[operation] = {
            'type': 'slowdown',
            'delay': delay_seconds
        }
    ...
    def simulate_operation(self, operation: str, func: Callable, *args, **kwargs):
        ...
        if 'memory' in self.performance_rules:
            ...
            if current_memory > rule['limit_mb']:
                raise MemoryError(f"Memory limit exceeded: {current_memory:.1f}MB > {rule['limit_mb']}MB")
...
@contextmanager
def error_simulation(condition: str, exception_class: type, message: str = None):
...
@contextmanager
def performance_simulation(operation: str = None, delay_seconds: float = 0.1):
