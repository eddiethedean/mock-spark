...
class MockStorageManager:
    ...
    def query_table(self, schema: str, table: str, filter_expr: Optional[str] = None) -> List[Dict[str, Any]]:
        """Query table data."""
        if not self.table_exists(schema, table):
            return []
    ...
    def drop_table(self, schema: str, table: str) -> None:
        ...
        conn = self.get_database(schema)
        conn.execute(f"DROP TABLE IF EXISTS `{table}`")  # Use backticks to avoid SQL keyword conflicts
