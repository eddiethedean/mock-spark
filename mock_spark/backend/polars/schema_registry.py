...
class SchemaRegistry:
    ...
    def save_schema(
        self, schema_name: str, table_name: str, schema: MockStructType
    ) -> None:
        ...
        for i, field in enumerate(schema.fields):
            if isinstance(field.dataType, ArrayType):
                schema_data["fields"][i]["elementType"] = (
                    field.dataType.elementType.__class__.__name__
                )
            elif isinstance(field.dataType, MapType):
                schema_data["fields"][i]["keyType"] = (
                    field.dataType.keyType.__class__.__name__
                )
                schema_data["fields"][i]["valueType"] = (
                    field.dataType.valueType.__class__.__name__
                )
    ...
    def load_schema(
        ...
        Returns:
            MockStructType if schema exists, None otherwise
        """
        schema_path = self._get_schema_path(schema_name, table_name)
        ...
        for field_data in schema_data["fields"]:
            ...
            if type_name == "ArrayType":
                ...
                data_type = ArrayType(elementType=element_type)
                ...
                data_type = MapType(keyType=key_type, valueType=value_type)
