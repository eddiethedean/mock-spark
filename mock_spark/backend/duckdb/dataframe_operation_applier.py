...
from sqlalchemy import Session, select, func, text, and_, or_, literal_column, literal
...
class DataFrameOperationApplier:
            ...
            if hasattr(col, "function_name"):
                ...
                if col.function_name in ["upper", "lower", "length", "trim"]:
                    ...
                    select_columns.append(
                        ...
                        else func.length(source_table_obj.c[col.column.name]).label(col_name)
                    ...
                    new_columns.append(Column("id", Float, primary_key=False))
                    select_columns.append(
                        func.abs(source_table_obj.c[col.column.name]).label(col_name)
                        if col.function_name == "abs"
                        else func.round(source_table_obj.c[col.column.name]).label(col_name)
                    )
                else:
                    # Other functions - use raw SQL
                    func_sql = self.sql_converter.expression_to_sql(col, source_table=source_table)
    ...
    def apply_with_column(
        ...
        if hasattr(col, "function_name"):
            ...
            if col.function_name in ["upper", "lower", "length", "trim"]:
                ...
                col_sql = self.sql_converter.expression_to_sql(col, source_table=source_table)
                select_columns.append(literal_column(col_sql).label(col_name))
                new_columns.append(Column(col_name, String, primary_key=False))
        else:
            ...
            if hasattr(col, "column"):
                ...
                from sqlalchemy import String
                new_columns.append(Column(col_name, String, primary_key=False))
                select_columns.append(literal(str(col)).label(col_name))
    ...
    def apply_union(
        self, source_table: str, target_table: str, other_df: Any
    ) -> None:
