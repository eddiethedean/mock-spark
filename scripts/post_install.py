...
def create_version_marker() -> None:
    ...
    if env_version:
        for extra, marker_file in version_extras.items():
            if env_version in extra:
                ...
                print(
                    f"   PySpark {env_version.replace('pyspark-', '').replace('-', '.')} API compatibility enabled"
                )
                marker_created = True
                break
    ...
    if not marker_created:
        for extra, marker_file in version_extras.items():
            if (
                f"[{extra}]" in install_args
                or f"'{extra}'" in install_args
                or f'"{extra}"' in install_args
            ):
                marker_path = package_dir / marker_file
                marker_path.touch(exist_ok=True)
