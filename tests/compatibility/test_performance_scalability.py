...
class TestPerformanceCharacteristics:
    ...
    def test_complex_operations_performance(self, mock_functions, pyspark_functions):
        ...
        ).filter(mock_functions.col("value1") > 100).collect()
        mock_time = time.time() - start_time
        ...
        pyspark_result = pyspark_df.select(
            ...
            (pyspark_functions.col("value2") * pyspark_functions.col("value3")).alias("product_23"),
            pyspark_functions.col("value1").alias("alias_1")
        ).filter(pyspark_functions.col("value1") > 100).collect()
        pyspark_time = time.time() - start_time
        ...
        print(f"Complex operations - Mock time: {mock_time:.4f}s, PySpark time: {pyspark_time:.4f}s")
...
class TestMemoryUsage:
    ...
    def test_memory_efficiency(self, mock_functions, pyspark_functions):
        ...
        ).filter(mock_functions.col("age") > 30).collect()
        ...
        ).filter(pyspark_functions.col("age") > 30).collect()
    ...
    def test_deep_operation_chaining(self, mock_functions, pyspark_functions):
        ...
        for i in range(20):
            result = result.select(
                mock_functions.col("*"),
                mock_functions.lit(i).alias(f"chain_{i}")
            )
        ...
        for i in range(20):
            result = result.select(
                pyspark_functions.col("*"),
                pyspark_functions.lit(i).alias(f"chain_{i}")
            )
