...
def real_spark():
    ...
    spark = (
        RealSparkSession.builder.appName("compatibility-test-3.0-3.1")
        .master("local[1]")
...
class TestHyperbolicMathCompat:
    ...
    def test_acosh_compat(self, real_spark, mock_spark):
        ...
        real_result = real_df.select(
            F_real.col("id"), F_real.acosh(F_real.col("value")).alias("result")
        ).collect()

        # Mock Spark
        mock_df = mock_spark.createDataFrame(data)
        mock_result = mock_df.select(
            F_mock.col("id"), F_mock.acosh(F_mock.col("value")).alias("result")
        ).collect()
    ...
    def test_asinh_compat(self, real_spark, mock_spark):
        ...
        real_df = real_spark.createDataFrame(data)
        real_result = real_df.select(
            F_real.col("id"), F_real.asinh(F_real.col("value")).alias("result")
        ).collect()

        # Mock Spark
        mock_df = mock_spark.createDataFrame(data)
        mock_result = mock_df.select(
            F_mock.col("id"), F_mock.asinh(F_mock.col("value")).alias("result")
        ).collect()
    ...
    def test_atanh_compat(self, real_spark, mock_spark):
        ...
        real_df = real_spark.createDataFrame(data)
        real_result = real_df.select(
            F_real.col("id"), F_real.atanh(F_real.col("value")).alias("result")
        ).collect()

        # Mock Spark
        mock_df = mock_spark.createDataFrame(data)
        mock_result = mock_df.select(
            F_mock.col("id"), F_mock.atanh(F_mock.col("value")).alias("result")
        ).collect()
...
class TestUtilityFunctionsCompat:
    ...
    @pytest.mark.skipif(
        not pyspark_has_function("make_date"), reason="make_date() added in PySpark 3.3"
    )
    def test_make_date_compat(self, real_spark, mock_spark):
        ...
        real_result = real_df.select(
            F_real.col("id"),
            F_real.make_date(
                F_real.col("year"), F_real.col("month"), F_real.col("day")
            ).alias("date"),
        ).collect()
        ...
        mock_result = mock_df.select(
            F_mock.col("id"),
            F_mock.make_date(
                F_mock.col("year"), F_mock.col("month"), F_mock.col("day")
            ).alias("date"),
        ).collect()
    ...
    @pytest.mark.skipif(
        not pyspark_has_function("version"), reason="version() added in PySpark 3.5"
    )
...
class TestBooleanAggregatesCompat:
    ...
    @pytest.mark.skipif(
        not pyspark_has_function("bool_and"), reason="bool_and() added in PySpark 3.5"
    )
    def test_bool_and_compat(self, real_spark, mock_spark):
        ...
        real_df = real_spark.createDataFrame(data)
        real_result = (
            real_df.groupBy("group")
            .agg(F_real.bool_and(F_real.col("flag")).alias("all_true"))
            .orderBy("group")
            .collect()
        )
        ...
        mock_df = mock_spark.createDataFrame(data)
        mock_result = (
            mock_df.groupBy("group")
            .agg(F_mock.bool_and(F_mock.col("flag")).alias("all_true"))
            .collect()
        )
    ...
    @pytest.mark.skipif(
        not pyspark_has_function("bool_or"), reason="bool_or() added in PySpark 3.5"
    )
    def test_bool_or_compat(self, real_spark, mock_spark):
        ...
        real_result = (
            real_df.groupBy("group")
            .agg(F_real.bool_or(F_real.col("flag")).alias("any_true"))
            .orderBy("group")
            .collect()
        )
        ...
        mock_result = (
            mock_df.groupBy("group")
            .agg(F_mock.bool_or(F_mock.col("flag")).alias("any_true"))
            .collect()
        )
        mock_result = sorted(mock_result, key=lambda r: r["group"])
    ...
    @pytest.mark.skipif(
        not pyspark_has_function("every"), reason="every() added in PySpark 3.5"
    )
    def test_every_alias_compat(self, real_spark, mock_spark):
        ...
        real_result = real_df.agg(
            F_real.every(F_real.col("flag")).alias("result")
        ).collect()

        # Mock Spark
        mock_df = mock_spark.createDataFrame(data)
        mock_result = mock_df.agg(
            F_mock.every(F_mock.col("flag")).alias("result")
        ).collect()
    ...
    @pytest.mark.skipif(
        not pyspark_has_function("some"), reason="some() added in PySpark 3.5"
    )
    def test_some_alias_compat(self, real_spark, mock_spark):
        ...
        real_df = real_spark.createDataFrame(data)
        real_result = real_df.agg(
            F_real.some(F_real.col("flag")).alias("result")
        ).collect()

        # Mock Spark
        mock_df = mock_spark.createDataFrame(data)
        mock_result = mock_df.agg(
            F_mock.some(F_mock.col("flag")).alias("result")
        ).collect()
...
class TestAdvancedAggregatesCompat:
    ...
    @pytest.mark.skipif(
        not pyspark_has_function("max_by"), reason="max_by() added in PySpark 3.3"
    )
    ...
    @pytest.mark.skipif(
        not pyspark_has_function("min_by"), reason="min_by() added in PySpark 3.3"
    )
    def test_min_by_compat(self, real_spark, mock_spark):
        ...
        real_df = real_spark.createDataFrame(data)
        real_result = real_df.agg(
            F_real.min_by(F_real.col("name"), F_real.col("score")).alias(
                "lowest_scorer"
            )
        ).collect()
        ...
        mock_df = mock_spark.createDataFrame(data)
        mock_result = mock_df.agg(
            F_mock.min_by(F_mock.col("name"), F_mock.col("score")).alias(
                "lowest_scorer"
            )
        ).collect()
    ...
    @pytest.mark.skipif(
        not pyspark_has_function("count_if"), reason="count_if() added in PySpark 3.5"
    )
    ...
    @pytest.mark.skipif(
        not pyspark_has_function("any_value"), reason="any_value() added in PySpark 3.5"
    )
    def test_any_value_compat(self, real_spark, mock_spark):
        ...
        real_result = (
            real_df.groupBy("group")
            .agg(F_real.any_value(F_real.col("value")).alias("some_value"))
            .orderBy("group")
            .collect()
        )
        ...
        mock_result = (
            mock_df.groupBy("group")
            .agg(F_mock.any_value(F_mock.col("value")).alias("some_value"))
            .collect()
        )
        mock_result = sorted(mock_result, key=lambda r: r["group"])
        ...
        for i in range(len(real_result)):
            ...
            group_values = [
                d["value"] for d in data if d["group"] == real_result[i]["group"]
            ]
...
class TestGroupedAggregatesCompat:
    ...
    def test_max_by_grouped_compat(self, real_spark, mock_spark):
        ...
        real_df = real_spark.createDataFrame(data)
        real_result = (
            real_df.groupBy("dept")
            .agg(
                F_real.max_by(F_real.col("name"), F_real.col("salary")).alias(
                    "top_earner"
                )
            )
            .orderBy("dept")
        ...
        mock_df = mock_spark.createDataFrame(data)
        mock_result = (
            mock_df.groupBy("dept")
            .agg(
                F_mock.max_by(F_mock.col("name"), F_mock.col("salary")).alias(
                    "top_earner"
                )
            )
            .collect()
    ...
    def test_count_if_grouped_compat(self, real_spark, mock_spark):
        ...
        real_df = real_spark.createDataFrame(data)
        real_result = (
            real_df.groupBy("category")
            .agg(F_real.count_if(F_real.col("value") > 20).alias("count_above_20"))
            .orderBy("category")
            .collect()
        )
        ...
        mock_df = mock_spark.createDataFrame(data)
        mock_result = (
            mock_df.groupBy("category")
            .agg(F_mock.count_if(F_mock.col("value") > 20).alias("count_above_20"))
            .collect()
        )
...
class TestEdgeCasesCompat:
    ...
    def test_bool_and_empty_group(self, real_spark, mock_spark):
        ...
        real_df = real_spark.createDataFrame(data)
        real_result = real_df.agg(
            F_real.bool_and(F_real.col("flag")).alias("result")
        ).collect()

        # Mock Spark
        mock_df = mock_spark.createDataFrame(data)
        mock_result = mock_df.agg(
            F_mock.bool_and(F_mock.col("flag")).alias("result")
        ).collect()
