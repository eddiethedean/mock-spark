...
class TestComplexDataTypes:
    ...
    def test_array_type_creation(self, mock_complex_dataframe, pyspark_complex_dataframe):
        ...
        try:
            ...
            mock_array_fields = [f for f in mock_schema.fields if 'array' in str(f.dataType).lower()]
            pyspark_array_fields = [f for f in pyspark_schema.fields if 'array' in str(f.dataType).lower()]
            
            assert len(mock_array_fields) == len(pyspark_array_fields), "Array type field count mismatch"
    ...
    def test_map_type_creation(self, mock_complex_dataframe, pyspark_complex_dataframe):
        ...
        try:
            ...
            pyspark_map_fields = [f for f in pyspark_schema.fields if 'map' in str(f.dataType).lower()]
    ...
    def test_struct_type_creation(self, mock_complex_dataframe, pyspark_complex_dataframe):
        ...
        try:
            ...
            mock_struct_fields = [f for f in mock_schema.fields if 'struct' in str(f.dataType).lower()]
            pyspark_struct_fields = [f for f in pyspark_schema.fields if 'struct' in str(f.dataType).lower()]
            
            assert len(mock_struct_fields) == len(pyspark_struct_fields), "Struct type field count mismatch"
...
class TestAdvancedSQLFunctions:
    ...
    def test_case_when_function(self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions):
    ...
    def test_coalesce_function(self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions):
        """Test coalesce() function."""
        try:
            mock_result = mock_dataframe.select(
                mock_functions.coalesce(mock_functions.col("name"), mock_functions.lit("Unknown")).alias("coalesced_name")
            )
            pyspark_result = pyspark_dataframe.select(
                pyspark_functions.coalesce(pyspark_functions.col("name"), pyspark_functions.lit("Unknown")).alias("coalesced_name")
            )
            assert_dataframes_equal(mock_result, pyspark_result)
        except (AttributeError, NotImplementedError):
            pytest.skip("coalesce() function not implemented in mock_spark yet")
    
    def test_isnull_function(self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions):
    ...
    def test_isnan_function(self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions):
        """Test isnan() function."""
        try:
            mock_result = mock_dataframe.select(
                mock_functions.col("*"),
...
class TestDateAndTimeFunctions:
    ...
    def test_current_timestamp_function(self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions):
    ...
    def test_current_date_function(self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions):
...
class TestStringAdvancedFunctions:
    ...
    def test_trim_function(self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions):
        """Test trim() function."""
        try:
            mock_result = mock_dataframe.select(
                mock_functions.col("*"),
    ...
    def test_regexp_replace_function(self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions):
        """Test regexp_replace() function."""
        try:
            mock_result = mock_dataframe.select(
                mock_functions.col("*"),
                mock_functions.regexp_replace(mock_functions.col("name"), "e", "X").alias("replaced_name")
            )
            pyspark_result = pyspark_dataframe.select(
                pyspark_functions.col("*"),
                pyspark_functions.regexp_replace(pyspark_functions.col("name"), "e", "X").alias("replaced_name")
            )
            assert_dataframes_equal(mock_result, pyspark_result)
        except (AttributeError, NotImplementedError):
            pytest.skip("regexp_replace() function not implemented in mock_spark yet")
    
    def test_split_function(self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions):
        """Test split() function."""
        try:
            mock_result = mock_dataframe.select(
                mock_functions.col("*"),
            ...
            pyspark_result = pyspark_dataframe.select(
                pyspark_functions.col("*"),
                pyspark_functions.split(pyspark_functions.col("department"), " ").alias("split_dept")
            )
            assert_dataframes_equal(mock_result, pyspark_result)
        except (AttributeError, NotImplementedError):
...
class TestMathematicalAdvancedFunctions:
    ...
    def test_ceil_function(self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions):
        """Test ceil() function."""
        try:
            mock_result = mock_dataframe.select(
                mock_functions.col("*"),
    ...
    def test_floor_function(self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions):
        """Test floor() function."""
        try:
            mock_result = mock_dataframe.select(
                mock_functions.col("*"),
    ...
    def test_sqrt_function(self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions):
        """Test sqrt() function."""
        try:
            mock_result = mock_dataframe.select(
                mock_functions.col("*"),
