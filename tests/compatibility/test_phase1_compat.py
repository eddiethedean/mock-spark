...
class TestForeachMethods:
    ...
    @pytest.mark.skip(reason="foreach side effects behave differently in distributed vs local execution")
...
class TestOptimizationMethods:
    ...
    def test_sort_within_partitions(self, mock_spark, real_spark, mock_functions, pyspark_functions):
        ...
        # Test with PySpark
        pyspark_df = real_spark.createDataFrame(data)
        pyspark_result = pyspark_df.sortWithinPartitions(pyspark_functions.col("value").desc())
...
class TestWatermark:
    ...
    def test_with_watermark_chainable(self, mock_spark, real_spark, mock_functions, pyspark_functions):
        ...
        # Test with mock
        mock_df = mock_spark.createDataFrame(data)
        mock_result = (mock_df.withWatermark("timestamp", "5 minutes")
                       .filter(mock_functions.col("value") > 0))
        
        # Test with PySpark
        pyspark_df = real_spark.createDataFrame(data)
        pyspark_result = (pyspark_df.withWatermark("timestamp", "5 minutes")
                          .filter(pyspark_functions.col("value") > 0))
...
class TestUDFFeatures:
    ...
    @pytest.mark.skip(reason="UDF execution differs between PySpark (Catalyst) and mock-spark (Python)")
    def test_udf_basic_usage(self, mock_spark, real_spark, mock_functions, pyspark_functions):
        ...
        mock_result = mock_df.select(mock_double(mock_functions.col("value")).alias("doubled"))
    ...
    def test_udf_decorator_pattern(self, mock_spark, real_spark, mock_functions, pyspark_functions):
        ...
        mock_df = mock_spark.createDataFrame(data)
        mock_result = mock_df.select(mock_upper(mock_functions.col("name")).alias("upper_name"))
        mock_data = mock_result.collect()
        assert len(mock_data) == 2
    ...
    def test_pandas_udf_basic(self, mock_spark, real_spark, mock_functions, pyspark_functions):
        ...
        pyspark_df = real_spark.createDataFrame(data)
        pyspark_result = pyspark_df.select(pyspark_double(pyspark_functions.col("value")).alias("doubled"))
