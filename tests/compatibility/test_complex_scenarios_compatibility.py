...
class TestComplexScenariosCompatibility:
    ...
    def test_withcolumn_filter_orderby_chain(self, mock_spark_session):
        ...
        expected = load_expected_output("chained_operations", "withcolumn_filter_orderby")
        assert_dataframes_equal(result, expected)
    ...
    def test_select_expr_groupby_agg_orderby_chain(self, mock_spark_session):
        ...
        df = mock_spark_session.createDataFrame(test_data)
        result = (df.selectExpr("name", "salary", "department", "CASE WHEN age > 30 THEN 'Senior' ELSE 'Junior' END as level")
                  .groupBy("level")
    ...
    def test_complex_string_operations_chain(self, mock_spark_session):
        ...
        df = mock_spark_session.createDataFrame(test_data)
        result = (df.select(F.upper(df.name).alias("name_upper"), 
                           F.concat(df.name, F.lit(" ("), df.department, F.lit(")")).alias("name_dept"), 
                           F.length(df.name).alias("name_length"))
                  .filter(F.length(df.name) > 4))
        
        expected = load_expected_output("chained_operations", "complex_string_operations")
        assert_dataframes_equal(result, expected)
