...
class TestStringFunctionsExtendedCompatibility:
    ...
    @pytest.mark.skip(reason="Encoding functions may require PySpark for exact comparison")
    ...
    @pytest.mark.skip(reason="Decoding functions may require PySpark for exact comparison")
    ...
    def test_get_json_object(self, spark):
        ...
        df = spark.createDataFrame(expected["input_data"])
        result = df.select(F.get_json_object(F.lit('{"name":"Alice","age":25}'), "$.name"))
        assert_dataframes_equal(result, expected)
    ...
    def test_json_tuple(self, spark):
        ...
        df = spark.createDataFrame(expected["input_data"])
        result = df.select(F.json_tuple(F.lit('{"name":"Alice","age":25}'), "name", "age"))
        assert_dataframes_equal(result, expected)
