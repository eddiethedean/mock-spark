    ...
    while i < len(lines):
        ...
        if current_bug:
            if line.startswith('**Status**:'):
                ...
                current_bug['files'] = [f.strip().strip('`') for f in files_str.split(',')]
                ...
                i += 1
                in_code_block = False
                while i < len(lines) and (lines[i].startswith('```') or in_code_block or (not lines[i].startswith('**') and not lines[i].startswith('---'))):
                ...
                current_bug['workaround'] = line.split(':', 1)[1].strip() if ':' in line else ''
...
def create_issue_body(bug):
    ...
    body_parts.append("")
    body_parts.append("**Source**: This issue was automatically created from `tests/BUG_LOG.md`")
...
def create_issue(bug, dry_run=True):
    ...
    try:
        cmd = ['gh', 'issue', 'create', '--title', title, '--body-file', body_file] + label_args
...
def main():
    ...
    parser.add_argument('--bug', help='Create issue for specific bug ID (e.g., BUG-001)')
