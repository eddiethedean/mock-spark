...
class TestDatetimeOperations(ParityTestBase):
    ...
    def test_hour_extraction(self, mock_spark, pyspark_spark, datetime_data):
        ...
        mock_result = mock_df.select("timestamp", MockF.hour("timestamp").alias("extracted_hour"))
        
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(datetime_data)
        pyspark_result = pyspark_df.select("timestamp", PySparkF.hour("timestamp").alias("extracted_hour"))
    ...
    def test_dayofweek_extraction(self, mock_spark, pyspark_spark, datetime_data):
        ...
        # MockSpark
        mock_df = mock_spark.createDataFrame(datetime_data)
        mock_result = mock_df.select("timestamp", MockF.dayofweek("timestamp").alias("day_of_week"))
        
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(datetime_data)
        pyspark_result = pyspark_df.select("timestamp", PySparkF.dayofweek("timestamp").alias("day_of_week"))
    ...
    def test_to_timestamp_without_format(self, mock_spark, pyspark_spark):
        ...
        mock_result = mock_df.select("timestamp_str", MockF.to_timestamp("timestamp_str").alias("timestamp"))
        
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(data)
        pyspark_result = pyspark_df.select("timestamp_str", PySparkF.to_timestamp("timestamp_str").alias("timestamp"))
    ...
    def test_to_timestamp_with_format(self, mock_spark, pyspark_spark):
        ...
        # MockSpark
        mock_df = mock_spark.createDataFrame(data)
        mock_result = mock_df.select("timestamp_str", MockF.to_timestamp("timestamp_str", "dd/MM/yyyy HH:mm:ss").alias("timestamp"))
        
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(data)
        pyspark_result = pyspark_df.select("timestamp_str", PySparkF.to_timestamp("timestamp_str", "dd/MM/yyyy HH:mm:ss").alias("timestamp"))
    ...
    def test_to_date(self, mock_spark, pyspark_spark):
        ...
        # MockSpark
        mock_df = mock_spark.createDataFrame(data)
        mock_result = mock_df.select("date_str", MockF.to_date("date_str").alias("date"))
        
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(data)
        pyspark_result = pyspark_df.select("date_str", PySparkF.to_date("date_str").alias("date"))
    ...
    def test_current_timestamp(self, mock_spark, pyspark_spark, sample_data):
        ...
        mock_result = mock_df.select("id", MockF.current_timestamp().alias("current_ts"))
        
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(sample_data)
        pyspark_result = pyspark_df.select("id", PySparkF.current_timestamp().alias("current_ts"))
    ...
    def test_current_date(self, mock_spark, pyspark_spark, sample_data):
        ...
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(sample_data)
        pyspark_result = pyspark_df.select("id", PySparkF.current_date().alias("current_date"))
    ...
    def test_date_format(self, mock_spark, pyspark_spark, datetime_data):
        ...
        # MockSpark
        mock_df = mock_spark.createDataFrame(datetime_data)
        mock_result = mock_df.select("timestamp", MockF.date_format("timestamp", "yyyy-MM-dd").alias("formatted_date"))
        
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(datetime_data)
        pyspark_result = pyspark_df.select("timestamp", PySparkF.date_format("timestamp", "yyyy-MM-dd").alias("formatted_date"))
    ...
    def test_year_extraction(self, mock_spark, pyspark_spark, datetime_data):
        ...
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(datetime_data)
        pyspark_result = pyspark_df.select("timestamp", PySparkF.year("timestamp").alias("year"))
    ...
    def test_month_extraction(self, mock_spark, pyspark_spark, datetime_data):
        ...
        # MockSpark
        mock_df = mock_spark.createDataFrame(datetime_data)
        mock_result = mock_df.select("timestamp", MockF.month("timestamp").alias("month"))
        
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(datetime_data)
        pyspark_result = pyspark_df.select("timestamp", PySparkF.month("timestamp").alias("month"))
    ...
    def test_day_extraction(self, mock_spark, pyspark_spark, datetime_data):
        ...
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(datetime_data)
        pyspark_result = pyspark_df.select("timestamp", PySparkF.day("timestamp").alias("day"))
    ...
    def test_date_add(self, mock_spark, pyspark_spark, datetime_data):
        ...
        # MockSpark
        mock_df = mock_spark.createDataFrame(datetime_data)
        mock_result = mock_df.select("date", MockF.date_add("date", 7).alias("date_plus_7"))
        
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(datetime_data)
        pyspark_result = pyspark_df.select("date", PySparkF.date_add("date", 7).alias("date_plus_7"))
    ...
    def test_date_sub(self, mock_spark, pyspark_spark, datetime_data):
        ...
        # MockSpark
        mock_df = mock_spark.createDataFrame(datetime_data)
        mock_result = mock_df.select("date", MockF.date_sub("date", 7).alias("date_minus_7"))
        
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(datetime_data)
        pyspark_result = pyspark_df.select("date", PySparkF.date_sub("date", 7).alias("date_minus_7"))
    ...
    def test_datediff(self, mock_spark, pyspark_spark, datetime_data):
        ...
        # MockSpark
        mock_df = mock_spark.createDataFrame(datetime_data)
        mock_result = mock_df.select("date", MockF.datediff("date", "2024-01-01").alias("days_since_jan_1"))
        
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(datetime_data)
        pyspark_result = pyspark_df.select("date", PySparkF.datediff("date", "2024-01-01").alias("days_since_jan_1"))
    ...
    def test_timestamp_cast_to_long(self, mock_spark, pyspark_spark, datetime_data):
        ...
        # MockSpark
        mock_df = mock_spark.createDataFrame(datetime_data)
        mock_result = mock_df.select("timestamp", mock_df.timestamp.cast("long").alias("epoch_seconds"))
        
        # PySpark
        pyspark_df = pyspark_spark.createDataFrame(datetime_data)
        pyspark_result = pyspark_df.select("timestamp", pyspark_df.timestamp.cast("long").alias("epoch_seconds"))
    ...
    def test_complex_datetime_operations(self, mock_spark, pyspark_spark, datetime_data):
