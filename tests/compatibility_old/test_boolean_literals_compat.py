...
class TestBooleanLiteralsCompatibility:
    ...
    def test_boolean_literal_true_compatibility(
        ...
        pyspark_result = pyspark_dataframe.select(
            pyspark_functions.lit(True).alias("is_active")
        )
    ...
    def test_boolean_literal_false_compatibility(
        ...
        mock_result = mock_dataframe.select(
            mock_functions.lit(False).alias("is_deleted")
        )
        pyspark_result = pyspark_dataframe.select(
            pyspark_functions.lit(False).alias("is_deleted")
        )
        assert_dataframes_equal(mock_result, pyspark_result)
    ...
    def test_boolean_literals_in_withcolumn(
        ...
        pyspark_result = pyspark_dataframe.withColumn(
            "active", pyspark_functions.lit(True)
        )
        assert_dataframes_equal(mock_result, pyspark_result)

        # Test adding false boolean columns
        mock_result = mock_result.withColumn("deleted", mock_functions.lit(False))
        pyspark_result = pyspark_result.withColumn(
            "deleted", pyspark_functions.lit(False)
        )
        assert_dataframes_equal(mock_result, pyspark_result)
    ...
    def test_boolean_literals_schema_compatibility(
        ...
        assert (
            type(mock_schema.fields[0].dataType).__name__
            == type(pyspark_schema.fields[0].dataType).__name__
        )
        assert (
            type(mock_schema.fields[1].dataType).__name__
            == type(pyspark_schema.fields[1].dataType).__name__
        )
    ...
    def test_boolean_literals_with_conditions(
        ...
        pyspark_result = pyspark_dataframe.select(
            pyspark_functions.when(
                pyspark_functions.col("id") > 2, pyspark_functions.lit(True)
            )
