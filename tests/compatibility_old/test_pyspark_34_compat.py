...
class TestMelt:
    ...
    def test_melt_multiple_ids(self, mock_spark, real_spark):
        ...
        data = [{"user": "Alice", "date": "2024-01-01", "clicks": 5, "views": 10}]
        ...
        try:
            pyspark_result = pyspark_df.melt(
                ids=["user", "date"], values=["clicks", "views"]
            )
            assert_dataframes_equal(mock_result, pyspark_result)
        except AttributeError:
            pytest.skip("melt() not available in this PySpark version")
    ...
    def test_melt_custom_column_names(self, mock_spark, real_spark):
        ...
        data = [{"id": 1, "metric1": 100, "metric2": 200}]
    ...
    def test_melt_preserves_types(self, mock_spark, real_spark):
        ...
        data = [{"id": 1, "score1": 95.5, "score2": 87.3}]
        ...
        try:
            ...
            mock_value_field = [
                f for f in mock_result.schema.fields if f.name == "value"
            ][0]
            pyspark_value_field = [
                f for f in pyspark_result.schema.fields if f.name == "value"
            ][0]
            assert (
                mock_value_field.dataType.typeName()
                == pyspark_value_field.dataType.typeName()
            )
        except AttributeError:
            pytest.skip("melt() not available in this PySpark version")
...
class TestToSchema:
    ...
    def test_to_with_schema_enforcement(self, mock_spark, real_spark):
        ...
        pyspark_schema = StructType(
            [StructField("a", PySparkIntType()), StructField("b", PySparkIntType())]
        )
        try:
    ...
    def test_to_drops_extra_columns(self, mock_spark, real_spark):
        ...
        from mock_spark.spark_types import (
            MockStructType,
            MockStructField,
            IntegerType as MockIntType,
            StringType as MockStringType,
        )
        from pyspark.sql.types import (
            StructType,
            StructField,
...
class TestWithMetadata:
    ...
    def test_with_metadata_basic(self, mock_spark, real_spark):
        ...
        try:
            ...
            pyspark_field = [f for f in pyspark_result.schema.fields if f.name == "id"][
                0
            ]
    ...
    def test_with_metadata_chainable(self, mock_spark, real_spark):
        ...
        # Test with mock
        mock_df = mock_spark.createDataFrame(data)
        mock_result = mock_df.withMetadata(
            "col1", {"desc": "First column"}
        ).withMetadata("col2", {"desc": "Second column"})

        # Test with PySpark
        pyspark_df = real_spark.createDataFrame(data)
        try:
            pyspark_result = pyspark_df.withMetadata(
                "col1", {"desc": "First column"}
            ).withMetadata("col2", {"desc": "Second column"})
...
class TestObserve:
    ...
    def test_observe_doesnt_change_data(
    ...
    def test_observe_is_chainable(
        ...
        mock_result = mock_df.observe(
            "metrics", mock_functions.count(mock_functions.lit(1))
        ...
        try:
            pyspark_result = pyspark_df.observe(
                "metrics", pyspark_functions.count(pyspark_functions.lit(1))
...
class TestIntegration:
    ...
    def test_melt_then_aggregate(
        ...
        data = [{"id": 1, "A": 10, "B": 20}, {"id": 2, "A": 15, "B": 25}]

        # Test with mock
        mock_df = mock_spark.createDataFrame(data)
        try:
            mock_melted = mock_df.melt(ids=["id"], values=["A", "B"])
            mock_result = mock_melted.groupBy("variable").agg(
        ...
        try:
            ...
            pyspark_result = pyspark_melted.groupBy("variable").agg(
    ...
    def test_observe_with_complex_aggregations(
        self, mock_spark, real_spark, mock_functions, pyspark_functions
    ):
