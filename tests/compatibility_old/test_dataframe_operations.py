...
class TestDataFrameCollect:
    ...
    def test_to_pandas_with_literals(
        self, mock_dataframe, pyspark_dataframe, mock_functions, pyspark_functions
    ):
...
class TestDiscrepancyDetection:
    ...
    @pytest.mark.skip(
        reason="Flaky test - non-deterministic ordering in groupBy/orderBy/limit sequence"
    )
    ...
    def test_edge_cases(self, mock_environment, pyspark_environment):
        ...
        assert len(mock_empty.schema.fields) == 0
