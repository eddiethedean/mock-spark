...
from mock_spark.spark_types import IntegerType, StringType, DoubleType, MockStructType, MockStructField
...
class TestToSchema:
    ...
    def test_to_schema_enforcement(self, spark):
        ...
        new_schema = MockStructType([
            MockStructField("x", IntegerType()),
            MockStructField("y", IntegerType())
        ])
...
class TestWithMetadata:
    ...
    def test_withmetadata_multiple_columns(self, spark):
        ...
        result = (df.withMetadata("col1", {"desc": "First column"})
...
class TestObserve:
    ...
    def test_observe_chainable(self, spark):
        ...
        result = (df.observe("metric1", F.count(F.lit(1)))
                   .observe("metric2", F.sum("id")))
