"""
Unit tests for boolean literal support in Mock-Spark 2.10.0.
...
class TestBooleanLiteralsUnit:
    ...
    def test_boolean_literals_aggregation(self):
        ...
        result = df.agg(
            F.lit(True).alias("all_true"),
            F.lit(False).alias("all_false")
        )
    ...
    def test_boolean_literals_schema_inference(self):
        ...
        result = df.select(
            F.lit(True).alias("bool_true"),
            F.lit(False).alias("bool_false")
        )
    ...
    def test_boolean_literals_edge_cases(self):
        ...
        result = df.select(
            F.lit(True).alias("not_null_bool"),
            F.lit(False).alias("false_bool")
        )
