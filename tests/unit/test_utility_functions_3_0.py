...
class TestUtilityFunctions3_0:
    ...
    @pytest.mark.skip(reason="overlay requires complex SQL syntax handling - implement in future release")
    def test_overlay_basic(self):
        """Test overlay - basic string replacement."""
        data = [
            {"id": 1, "text": "Hello World"},
            {"id": 2, "text": "PySpark is great"}
        ]
        ...
        result = df.select(
            F.col("id"),
            F.overlay(F.col("text"), F.lit("Friend"), F.lit(7), F.lit(5)).alias("replaced")
        ).collect()
