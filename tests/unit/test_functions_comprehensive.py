...
class TestFunctionsComprehensive:
    ...
    def test_math_functions_comprehensive(self, spark, test_data):
        ...
        result = df.select(
            ...
            F.exp(F.col("salary")).alias("exp_salary"),  # Simplified to avoid MockColumnOperation
        )
    ...
    def test_complex_expressions_comprehensive(self, spark, test_data):
        ...
        result = df.select(
            ...
            F.concat(F.col("name"), " (", F.col("department"), ")").alias("full_name"),
            # Complex math
    ...
    def test_edge_cases_and_error_handling(self, spark):
        ...
        result2 = single_row_df.select(F.col("value"), F.lit("test").alias("literal"))
