...
class TestWithWatermark:
    ...
    def test_withWatermark_chainable(self, spark):
        ...
        df = spark.createDataFrame([
            {"event_time": "2024-01-01", "count": 10}
        ])
    ...
    def test_withWatermark_with_groupby(self, spark):
        ...
        result = (df.withWatermark("timestamp", "1 day")
                   .groupBy("category")
...
class TestPandasUDF:
    ...
    def test_pandas_udf_with_string_return_type(self, spark):
        ...
        # Test applying it to a column
        df = spark.createDataFrame([{"name": "test"}])
...
class TestIntegration:
    ...
    def test_checkpoint_and_repartition(self, spark):
        ...
        result = (df.repartitionByRange(3, "value")
                   .localCheckpoint()
                   .select("value"))
    ...
    def test_all_iteration_methods(self, spark):
        ...
        count2 = {"n": 0}
        df.foreachPartition(lambda partition: count2.update({"n": count2["n"] + sum(1 for _ in partition)}))
        assert count2["n"] == 5
