...
class TestArrayFunctions:
    ...
    def test_array_basic(self):
        ...
        result = df.select(F.array(F.col("a"), F.col("b"), F.col("c")).alias("arr")).collect()
    ...
    def test_sort_array_descending(self):
        ...
        result = df.select(F.sort_array(F.col("values"), asc=False).alias("sorted")).collect()
    ...
    def test_array_multiple_types(self):
        ...
        spark = MockSparkSession("test")
        df = spark.createDataFrame([
            {"int_val": 1, "str_val": "a", "float_val": 1.5}
        ])
        
        # Test array with same-type columns
        result = df.select(F.array(F.col("int_val"), F.col("int_val")).alias("arr")).collect()
